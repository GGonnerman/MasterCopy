function encrypt(encoding) {
    ...
    if (encoding) {
        document.getElementById("data").value = encode(message);
        if (encoding) {
            copyToClipboard();
        }
    } else if (document.getElementById("pwrd").value == decode("Z3R/cXB5cX5+eHZs")) {
        document.getElementById("data").value = decode(message);
        if (encoding) {
            copyToClipboard();
        }
    }
